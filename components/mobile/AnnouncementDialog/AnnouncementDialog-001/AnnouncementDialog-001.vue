<template>
  <v-dialog v-if="list.length > 0" ref="announcement" v-model="openStatus" max-width="600px" content-class="announcement-dialog">
    <v-toolbar :dark="dark" color="primary" height="50px" class="dialogHeadet">
      <v-layout>
        <v-flex xs12>
          <v-toolbar-title class="headline text-center">
            <v-img src="./src/img/4.png" />
            &nbsp;&nbsp;{{ textFilter('title') }}</v-toolbar-title
          >
        </v-flex>
      </v-layout>
      <v-btn icon @click="close()" class="closeBtn">
        <v-icon>close</v-icon>
      </v-btn>
    </v-toolbar>
    <v-card>
      <v-card-text class="custom-card-text">
        <v-container fluid grid-list-md class="content">
          <v-tabs class="mt-5" v-model="currentType" fixed-tabs>
            ...
            <v-tab-item value="tab_all">
              <v-expansion-panels v-model="panel" :disabled="disabled">
                ...
                <v-flex px-5 class="checkbox">
                  <v-checkbox @change="check" v-model="checkBox" hide-details class="ma-0 pa-0" :label="textFilter('labelTex2')"></v-checkbox>
                </v-flex>
              </v-expansion-panels>
            </v-tab-item>
            <v-tab-item v-for="(type, i) in types" :key="i" :value="'tab_' + i">
              <v-expansion-panels v-model="panel" :disabled="disabled" multiple>
                <v-layout v-if="Boolean(currentList.length)" wrap ma-0>
                  <v-expansion-panel :ref="`tab_${i}_menu`">
                    <v-expansion-panel-header>
                      <v-card>
                        <v-card-title v-for="item in currentList" :key="item.Id" :class="[{ active: isActive(item.Id) }]" @click="changeItem(item)">
                          <v-toolbar height="40">
                            <v-toolbar-title class="title-date my-0"> {{ item.StartTime | formatD }} {{ item.Title }}</v-toolbar-title>
                            <v-spacer></v-spacer>
                          </v-toolbar>
