<template>
  <div class="Footer-default-wrap">
    <div class="Footer-default py-5" :class="footerClass">
      ...
      <!-- <div class="d-flex">
        <div class="d-flex license__img2" v-for="(item, index) in license2List" :key="index">
          <img class="image" :src="item.src" />
          <span class="align-self-center">{{ $t(item.text) }}</span>
        </div>
      </div> -->
    </div>
...
export default {
  ...
  methods: {
    ...
    textFilter(text, slot = '') {
      return textFilter(text, 'com_footer_default_', slot);
    },
  },
};
</script>
