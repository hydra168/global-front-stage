<template>
  <div id="Login-page" class="Login-001">
    <video
        ...
        <v-card class="page-inner">
          ...
          <div class="boxes">
              ...
              <v-form class="form" ref="form">
                ...
                <div v-if="!isLoginMode">
                  ...
                  <v-text-field
                    ...
                    required
                  ></v-text-field>
                  <div class="sms-code" v-if="isRegisterSMSVerify">
            ...
            <div :class="`box right-box ${!isLoginMode ? 'isRegister' : ''}`">
              <div>
                <v-btn block large depressed color="primary" :disabled="isLoading" @click="routerPush">
                  {{ textFilter(isLoginMode ? 'freeRegistration' : 'hasAccount') }}
                </v-btn>
              </div>
              <div class="liveChatButton">
                <v-btn block large depressed color="primary" :disabled="isLoading" @click="SHOW_LIVE_CHAT(0)">
                  {{ textFilter('customerService') }}
                </v-btn>
              </div>
              <div v-if="SHOW_GUEST_LOGIN_BTN" class="guestLoginButton">
                <v-btn block large depressed color="primary" :disabled="isLoading" @click="ROUTER_PUSH('/guest')">
                  {{ textFilter('guestLogin') }}
                </v-btn>
              </div>
              <div class="appDownloadButton">
                <v-menu :close-on-content-click="false" :open-on-hover="true" :top="false" offset-y max-width="30%">
                  <template v-slot:activator="{ on }">
                    <v-btn block large depressed color="primary" :disabled="isLoading" v-on="on || true">
...
import { PC_LOGIN_BG_SRC, PC_LOGIN_LOGO_SRC, PC_LOGIN_BG_VIDEO_SRC, SHOW_GUEST_LOGIN_BTN } from '@G/consts';
...
export default {
  ...
  methods: {
    ...mapActions(['CHECK_REMEMBER', 'GET_HELPER_DETAIL', 'POST_LOGIN', 'POST_REGISTER', 'SHOW_LIVE_CHAT', 'GET_CAPTCHA', 'ROUTER_PUSH']),
...
.Login-001 ::v-deep .login-page-wrap {
