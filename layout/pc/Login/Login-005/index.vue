<template>
  <div id="Login-page" class="Login-005">
    <!-- 登入後動畫 -->
    <MyTransition :aniType="'slide-down-custom'">
      <template v-if="isShowAni">
        <div :class="[{ loaded: isShowAni }, 'loading']" @click="isShowAni = false">
          <img :src="loginCoverBg" class="loading" />
          <!-- 備用:可新增標題 -->
          <!-- <h1 class="glowInOut animate__animated animate__infinite"></h1> -->
        </div>
      </template>
    </MyTransition>
    <video
    ...
    <v-card class="login-page-wrap" :style="staticBg">
      <v-container class="page">
        <v-card class="page-inner">
          <div class="boxes">
            <div class="box left-box" :class="{ isRegister: !isLoginMode }">
              ...
              <v-text-field
                ...
                dark
              ></v-text-field>
              <v-checkbox dark v-model="Remember">
                <div v-if="isLoginMode" slot="label">{{ textFilter('rememberText') }}</div>
                ...
                <v-menu :close-on-content-click="false" :open-on-hover="true" :top="false" offset-y max-width="30%">
                  <template v-slot:activator="{ on }">
                    <v-btn block color="primary" large outlined :disabled="isLoading" v-on="on || true">
                      <span :class="`${textColor}--text`">{{ textFilter('appDownload') }}</span>
                    </v-btn>
                  </template>
                  <v-card>
                    <v-list>
            ...
            <div :class="`box right-box `">
              <img class="logo pa-1 ma-2 mb-10" :src="logoUrlHandler" alt="logo" />
              <div>
                <v-btn id="btn-primary" block color="primary" large outlined :disabled="isLoading" @click="routerPush">
                  <span :class="`${textColor}--text`">{{ textFilter('freeRegistration') }}</span>
                </v-btn>
              </div>
              <div class="liveChatButton">
                <v-btn block color="primary" large outlined :disabled="isLoading" @click="SHOW_LIVE_CHAT(0)">
...
import { PC_LOGIN_BG_SRC, PC_LOGIN_LOGO_SRC, PC_LOGIN_BG_VIDEO_SRC, PC_LOGIN_COVOR_SRC } from '@G/consts';
import MyTransition from '@COM/MyTransition.vue';
...
.Login-005 ::v-deep .login-page-wrap {
...
.loading {
  ...
  opacity: 1;
  &.loaded {
    // animation: 1s fadeOut forwards 3s;
  }
  h1 {
    position: absolute;
    bottom: 30px;
...
@keyframes glowInOut {
  ...
  50% {
    text-shadow: 0 0 6px rgba(253, 248, 238, 0.5), 0 0 10px rgba(228, 211, 163, 0.5), 0 0 16px rgba(228, 211, 163, 0.5),
      0 0 20px rgba(228, 211, 163, 0.5);
  }
  100% {
    text-shadow: 0 0 6px #fdf8ee, 0 0 10px #e4d3a3, 0 0 16px #e4d3a3, 0 0 20px #e4d3a3;
  }
...
.glowInOut {
  -webkit-animation-name: glowInOut;
