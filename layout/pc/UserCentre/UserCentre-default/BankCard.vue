            ...
            <v-form ref="form" v-model="valid">
              ...
              <template v-if="bankCardType === 1">
                ...
                <v-text-field
                  ...
                  type="text"
                  @input.native="
                    cardForm.Card = cardForm.Card.replace(/\s+/g, '');
                    CHECK_CARD_NO(cardForm.Card);
                  "
                  :rules="cardRules"
                ></v-text-field>
                {{ cardValidation }}
...
export default {
  ...
  computed: {
    ...mapGetters(['isLoading', 'userProfileData', 'userBankList', 'config', 'locale', 'bankList', 'usdtAppChannels', 'cardValidation']),
    localeBankList() {
  ...
  methods: {
    ...mapActions(['GET_USER_PROFILE', 'GET_USER_BANK_LIST', 'POST_USER_BANK', 'CHECK_CARD_NO']),
