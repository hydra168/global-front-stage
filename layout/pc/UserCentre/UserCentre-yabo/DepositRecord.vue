  ...
  <div class="DepositRecord">
    ...
    <v-row justify="space-between" align="center">
      ...
      <v-col class="text-right">
        <v-btn outlined color="primary" class="mr-2" @click="resetSearchData()">{{ $t('g_common_searchBar_resetBtn') }}</v-btn>
        ...
        <template v-if="tableData.length" v-slot:body="{ items }">
          <tbody>
            <tr v-for="item in items" :key="item.name">
              ...
              <td>{{ item.Sn }}</td>
              <td>{{ paymentsTypeList[item.PayType] }}</td>
              <td>
                <v-chip v-if="depositStatusList[item.Status]" label :color="depositStatusList[item.Status].color" text-color="white" small>
                  {{ $t(depositStatusList[item.Status].title) }}
                </v-chip>
                <v-chip v-else label :color="depositStatusList['-1'].color" text-color="white" small>
                  {{ status['-1'].title }}
                </v-chip>
              </td>
