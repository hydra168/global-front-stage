  ...
  <div class="WithdrawalsRecord">
    ...
    <v-row justify="space-between" align="center">
      ...
      <v-col class="text-right">
        <v-btn color="primary" class="mr-2" outlined @click="resetSearchData()">{{ $t('g_common_searchBar_resetBtn') }}</v-btn>
        ...
        <template v-if="tableData.length" v-slot:body="{ items }">
          <tbody>
            <tr v-for="item in items" :key="item.name">
              ...
              <td>{{ item.Sn }}</td>
              <td>{{ $t(withdrawalsTypeList[item.Type].title) }}</td>
              <td class="text-center">
                ...
                <v-chip v-else label :color="withdrawalsStatusList['-1'].color" text-color="white" small>
                  {{ status['-1'].title }}
                </v-chip>
              </td>
