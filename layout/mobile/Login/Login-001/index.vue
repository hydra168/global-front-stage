<template>
  <v-container id="Login-page" class="m-login-page-wrap Login-001" :style="`background-image:url(${loginBgSrc})`">
    <v-card class="page-inner">
      <h1 v-if="config?.show_login_page_title === '1'" class="logo pa-1 mt-10">{{ webTitle }}</h1>
      <img v-else class="logo pa-1 mt-10" :src="logoUrlHandler" alt="logo" />
      <v-form ref="form">
        <v-text-field
          dense
          solo
        ...
        <div style="display: flex; justify-content: space-between">
          <v-checkbox v-model="Remember">
            ...
            <div v-else slot="label" @click="getHelperDetail">
              <span class="rememberText"> {{ textFilter('agreeText') }} {{ textFilter('agreeItemText') }}</span>
            </div>
          </v-checkbox>
          <v-btn rounded color="primary" class="mt-4 ml-8 liveChatButton" @click="SHOW_LIVE_CHAT(0)">
        ...
        <v-btn block rounded large depressed color="primary" class="mb-3" :loading="isLoading" @click="postHandle">
          {{ textFilter(isLoginMode ? 'loginBtn' : 'registerBtn') }}
          <span slot="loader">{{ textFilter(isLoginMode ? 'loggingText' : 'submittingText') }}</span>
        </v-btn>
        <v-btn block rounded large depressed color="primary" :disabled="isLoading" @click="routerPush">
        ...
        <v-btn
          ...
          depressed
          color="secondary"
          class="mt-3 appDownload appDownloadButton"
          :disabled="isLoading"
