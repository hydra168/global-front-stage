<template>
  <v-container id="Login-page" class="m-login-page-wrap Login-001" :style="`background-image:url(${loginBgSrc})`">
    <v-card outlined flat class="page-inner">
      <img class="logo pa-1 mt-10" :src="logoUrlHandler" alt="logo" />
      <v-form ref="form">
        <v-text-field
          v-model="Name"
        ...
        <div v-if="!isLoginMode">
          ...
          <v-text-field
            ...
            required
          ></v-text-field>
          <v-text-field v-if="!!Number(config.register_qq)" v-model="Qqlink" prepend-icon="send_to_mobile" :label="textFilter('QqlinkLabel')" />
        </div>
        <v-text-field
          v-if="showVerify"
          v-model="Value"
          ...
          <v-checkbox v-model="Remember">
            ...
            <div v-else slot="label" @click="getHelperDetail">
              <span class="rememberText">
                {{ textFilter('agreeText') }} <br />
                {{ textFilter('agreeItemText') }}
              </span>
            </div>
          </v-checkbox>
          <v-btn rounded depressed class="ml-10 liveChatButton linearGradient" @click="SHOW_LIVE_CHAT(0)">
            {{ $t('g_com_login_default_customService') }}
          </v-btn>
        </div>
        <div v-if="showVerify && captcha.b64s" @click="GET_CAPTCHA">
          <img :src="captcha.b64s" alt="" />
        </div>
        <div v-if="config.is_entry_tips_show === '1'" class="my-0">
        ...
        <v-btn block rounded large depressed class="mb-3 linearGradient" :loading="isLoading" @click="postHandle">
        ...
        <v-btn
          v-if="!isReactNativeApp"
          block
          rounded
          large
          depressed
          class="mt-3 grey darken-1 appDownload appDownloadButton"
          :disabled="isLoading"
          @click="GET_APP_DOWNLOAD()"
        >
...
import { MOBILE_LOGIN_BG_SRC, MOBILE_LOGIN_LOGO_SRC } from '@G/consts';
...
export default {
  ...
  computed: {
    ...mapGetters(['isLoading', 'rememberData', 'referralCode', 'registerStatus', 'helperDetail', 'config', 'captcha', 'isReactNativeApp']),
    isLoginVerify() {
  ...
  methods: {
    ...mapActions(['CHECK_REMEMBER', 'GET_HELPER_DETAIL', 'POST_LOGIN', 'POST_REGISTER', 'SHOW_LIVE_CHAT', 'GET_CAPTCHA', 'GET_APP_DOWNLOAD']),
...
.m-login-page-wrap {
  ...
  .linearGradient {
    background-image: linear-gradient(to right, #00c4ff, #12ffff);
