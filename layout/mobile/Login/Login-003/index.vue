<template>
  <v-container id="Login-page" class="m-login-page-wrap" :style="`background-image:url(${loginBgSrc})`">
    ...
    <v-card class="login-dialog pa-5">
      <!-- tab -->
      <div class="d-flex justify-space-between px-7 pb-3">
        <div class="pa-2 login-tab-item" :class="{ 'tab-select': !isLoginMode }" @click="isLoginMode = false">{{ textFilter('registerBtn') }}</div>
        <div class="pa-2 login-tab-item" :class="{ 'tab-select': isLoginMode }" @click="isLoginMode = true">{{ textFilter('loginBtn') }}</div>
      </div>
      <!-- 登入用頁面 -->
      <div v-if="isLoginMode">
        ...
        <div class="btns">
          <div>
            <v-btn block color="#818185" outlined :disabled="isLoading" @click="SHOW_LIVE_CHAT(0)"> {{ textFilter('customService') }} </v-btn>
          </div>
          <div v-if="SHOW_GUEST_LOGIN_BTN" class="guestLoginButton">
            <v-btn block color="#818185" outlined :disabled="isLoading" @click="ROUTER_PUSH('/guest')"> {{ textFilter('guestLogin') }} </v-btn>
          </div>
          <div>
            <v-btn block color="#818185" outlined :disabled="isLoading" @click="GET_APP_DOWNLOAD()"> {{ textFilter('appDownload') }} </v-btn>
            <!-- <v-menu :close-on-content-click="false" :open-on-hover="true" :top="false" offset-y max-width="30%">
              <template v-slot:activator="{ on }">
                <v-btn block color="#818185" outlined :disabled="isLoading" v-on="on || true">
      ...
      <div v-if="!isLoginMode">
        ...
        <v-checkbox v-model="Remember" dense class="my-0">
        ...
        <div class="btns">
          ...
          <div v-if="SHOW_GUEST_LOGIN_BTN" class="guestLoginButton">
            <v-btn block color="#818185" outlined :disabled="isLoading" @click="ROUTER_PUSH('/')"> {{ textFilter('guestLogin') }} </v-btn>
          </div>
...
import { MOBILE_LOGIN_BG_SRC, MOBILE_LOGIN_LOGO_SRC, SHOW_GUEST_LOGIN_BTN } from '@G/consts';
