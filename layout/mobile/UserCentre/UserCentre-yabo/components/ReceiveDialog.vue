  ...
  <v-dialog v-model="dialog" persistent max-width="290px">
    <v-card>
      <v-toolbar dark color="primary">
        <v-spacer></v-spacer>
        <v-toolbar-title class="headline">{{ textFilter('title') }}</v-toolbar-title>
        <v-spacer></v-spacer>
        <!-- <v-menu bottom left>
      ...
      <v-card-text>
        <v-container grid-list-md>
          <v-layout wrap text-center>
            <v-flex xs12>
              <v-card-text>{{ textFilter('content') }}</v-card-text>
            </v-flex>
            <v-flex xs12>
...
export default {
  ...
  methods: {
    ...
    textFilter(text) {
      return textFilter(text, 'layout_userCentre_default_com_receiveDialog_');
    },
  },
};
</script>
