<template>
  <v-container id="Deposit" class="Deposit">
    <div class="headerBox">
      <div class="navigate"><span>存款</span></div>
      <!-- 主錢包 -->
      <div class="hRightBox">
        <span v-if="balanceAllInfo['0'] && loadBalanceErrorCount <= 3">
          <p>{{ balanceAllInfo['0'].balance | money }}</p>
        </span>
        <span v-if="loadBalanceErrorCount > 3">{{ textFilter('loadingError') }}</span>
        <span :class="userInfoClass" div v-else-if="isLogin">
          <p>{{ userInfo.name }}</p>
        </span>
        <span class="viplevel">
    ...
    <!-- <v-card>
      <v-tabs dark background-color="teal darken-3">
        ...
        <v-tab v-for="(item, k) in paymentAccountList" :key="k" @click="selectPay(item)">{{ item.bank }} </v-tab>
      </v-tabs>
    </v-card> -->
    <v-list v-show="step == 1">
      <v-list-item
        v-for="(item, k) in paymentAccountList.filter(item => filterType.includes(Number(item.type)))"
        :key="k"
      ...
      >
        ...
        <v-list-item-icon>
          <v-icon size="15">arrow_forward_ios</v-icon>
        </v-list-item-icon>
      </v-list-item>
    ...
    <v-card v-show="step == 2">
      <v-card-text>
        ...
        <template v-else>
          ...
          <template v-else>
            <v-text-field
              ...
              @keyup="resultAmount = resultAmount.replace(/^(0+)|[^\d]+/g, '')"
              required
            ></v-text-field>
            <v-text-field
              readonly
        ...
        <v-btn block outlined color="#07AF4C" class="mt-3" @click="defualtSet">{{ textFilter('cancelBtn') }}</v-btn>
    ...
    <v-card v-show="step == 3">
      ...
      <div class="text-xs-center">
        <v-btn :loading="isLoading" color="#07AF4C" depressed @click="showQrcode = true">
          {{ textFilter('qRCodeBtn') }}
        </v-btn>
        <v-btn :loading="isLoading" color="#07AF4C" depressed @click.native="openApp()" v-if="onlineAppUrl != ''">
          {{ textFilter('appBtn') }}
        </v-btn>
        <v-btn :loading="isLoading" color="#07AF4C" depressed @click.native="routerPush()">
    ...
    <v-card v-show="step == 4">
      ...
      <div class="text-xs-center">
        <v-btn :loading="isLoading" color="#07AF4C" dark depressed @click.native="routerPush"> {{ textFilter('confirmBtn') }} </v-btn>
      </div>
    ...
    <v-card v-show="step == 5">
      ...
      <div class="text-xs-center">
        <v-btn :loading="isLoading" color="#07AF4C" dark depressed @click.native="routerPush">
      ...
      <v-card-text>
        ...
        <v-row class="justify-center">
          <v-col cols="6" class="py-0">
            <v-btn class="mb-5" block color="#07AF4C" dark depressed @click="copyInputValueToClipboard($refs.infoCard)"> 复制收款地址 </v-btn>
          </v-col>
        </v-row>
        <div class="d-flex justify-center mb-5">
          <vue-qr :text="depositOrderInfo.card" :size="200" :margin="20"></vue-qr>
        </div>
        <v-btn block :loading="isLoading" color="#07AF4C" dark depressed @click.native="routerPush">
          {{ textFilter('confirmBtn') }}
        </v-btn>
      </v-card-text>
    </v-card>
    ...
    <v-card v-if="step == 7">
      <v-card-text>
        ...
        <v-btn block :loading="isLoading" color="#07AF4C" depressed @click.native="routerPush"> {{ textFilter('confirmBtn') }} </v-btn>
      </v-card-text>
    </v-card>
...
export default {
  ...
  watch: {
    ...
    resultAmount(val) {
      const amountNum = val * this.currencyRate;
      // this.resultAmount = Math.floor(amountNum * 100) / 100;
      this.chargeNum = amountNum.toFixed(2);
    },
  },
  created() {
  ...
  mounted() {
    ...
    if (routeName === 'deposit') {
      this.SET_CURRENT_MENU_TITLE('存款');
    }
  },
  methods: {
    ...mapActions([
    ...
    selectPay(id) {
      ...
      this.banks = [];
      this.step++;
      this.onlineCharge = this.paymentAccountList.find(item => item.id === id);
    ...
    createOrderOnlineHandler(data) {
      ...
      if (result.other != null) {
        ...
        } else if (result.other.FORM != '') {
          if (this.isReactNativeApp) {
            // const openUrl = `${location.protocol}//${location.host}/#/pay/${escape(encodeURIComponent(result.other.FORM))}`;
            const openUrl = `${location.protocol}//${location.host}/#/pay/${result.other.FORM}`;
            this.$router.push(`/deposit?hwaoPayMethod=${openUrl}`);
          } else if (this.isNativeApp) {
            // const openUrl = `${location.protocol}//${location.host}/#/pay/${escape(encodeURIComponent(this.order.other.FORM))}`;
            const openUrl = `${location.protocol}//${location.host}/#/pay/${this.order.other.FORM}`;
            this.NATIVE_OPEN_URL(openUrl);
          } else {
            this.payForm = result.other.FORM.replace(/[_]blank/, '_self');
            this.showForm = true;
          }
...
.Deposit {
  ...
  .headerBox {
    ...
    .arrawLeft::before {
      ...
      width: 8px;
      height: 15px;
      background: url('./assets/img/right-arrow.png') no-repeat center center/cover;
